...
MyFriend 的 FastAPI 后端。严格分层：**Controller → Service → Repository → Model/DB**，调用方向单向。横切能力（RAG / 记忆 / Agent / LLM / 存储）放在 `core/` 下按子系统分目录。
...
MyFriend 不在 `.env` 里写任何 LLM 的 API Key。**所有模型与其 API Key 都由登录用户在前端「模型配置」页动态添加**，按用户隔离、存进数据库（`model_configs` 表）。这样多用户各用各的 Key，互不可见。
